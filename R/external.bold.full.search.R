...
#' bold.data.taxo.geo <- bold.full.search(
...
#' bold.data.taxo.geo.marker <- bold.full.search(
#'   taxonomy = list("Poecilia reticulata"),
...
#' df_test <- data.frame(
#'   taxon_name = c("Panthera leo", "Panthera uncia"),
#'   locations = c("India", "Sri Lanka")
#' )
#'
#' # Result (correct way)
#' bold.data.taxo.geo.df.col <- bold.full.search(
...
#'
#' bold.data.taxo.geo.df.col <- bold.full.search(
#'   taxonomy = list(df_test$taxon_name),
...
bold.full.search <- function(taxonomy = NULL,
  ...
  if (is.null(marker) && (!is.null(marker_min_length) || !is.null(marker_max_length))) {
    ...
    marker_final <- setNames(
      list(list(
        min = marker_min_length,
  ...
  suppressMessages(json_content_step3_text1 <- httr::content(
    step3,
    "text"
  ))
  ...
  input_data <- data.frame(
    processid = downloaded_ids_fordf,
