...
bold.multirecords.set.csv <- function(edited.json) {
  lapply(edited.json, function(x) {
    columns_to_collapse <- c(
      "bold_recordset_code_arr", "coord", "marker_code",
...
reassign.data.type <- function(x) {
  ...
  x[date.fields.intersectn] <- lapply(x[date.fields.intersectn], function(x) {
    as.Date(x, "%Y-%m-%d")
  })
