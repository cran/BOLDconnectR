...
bin.dataset.project.pids <- function(get.data.input,
  ...
  suppressMessages(
    suppressWarnings(
      json_bin_dataset_project_cont <- content(
        get.data,
        "text"
      )
    )
  ...
  json_bins_datasets_project_data <- lapply(
    strsplit(
      json_bin_dataset_project_cont,
      "\n"
  ...
  json_bins_datasets_project_pids <- lapply(
    strsplit(
      json_bin_dataset_project_cont,
      "\n"
    )[[1]], # split the content (here each process or sample id)
