...
bold.fetch.filters <- function(bold.df,
  ...
  if (!is.null(latitude)) {
    ...
    if (length(latitude) != 2) stop("Latitude should be a range separated by a comma (min lat, max lat).")
  ...
  if (!is.null(longitude)) {
    ...
    if (length(longitude) != 2) stop("Longitude should be a range separated by a comma (min lon, max lon).")
  ...
  if (!is.null(shapefile)) {
    ...
    spatial.bold.df <- suppressWarnings(suppressMessages(st_transform(
      spatial.bold.df,
      st_crs(shp_input)
    )))
    bold.df <- suppressWarnings(suppressMessages(st_intersection(
      spatial.bold.df,
      shp_input
    ) %>%
      sf::st_drop_geometry(.) %>%
  ...
  if (!is.null(collection.period)) {
    if (length(collection.period) < 2) {
      stop(
        "Collection period should be a range of date data type separated by a comma (start date, end date)."
      )
    }
    if (
      is.na(as.Date(collection.period[1], "%Y-%m-%d")) ||
        is.na(as.Date(collection.period[2], "%Y-%m-%d"))
    ) {
      stop("Collection period should be a date object in %Y-%m-%d format.")
    }
