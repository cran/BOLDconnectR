...
bold.public.search <- function(taxonomy = NULL,
  ...
  if (length(non_null_args) > 1) {
    message(
      paste0(red_col, "Downloading ids.", reset_col, "\r"),
      appendLF = FALSE
    )
    # Parsing the query
    step1 <- tryCatch(
      parse_query(trial_query_input),
      error = function(e) {
    ...
    # Correct argument name and query term association check
    parameter_validation(
      step3,
      non_null_args
    )
    # Function checks whether the input query terms entered are in their respective parameter arguments
    # Generate the query id
    step4 <- tryCatch(
      generate_query_id(step3),
      error = function(e) {
        stop(conditionMessage(e), call. = FALSE)
      }
    )
    # Download the data using the query id
    result <- obtain_data(step4)
    ...
    step3 <- tryCatch(
      counts_query(step2),
      # Function checks whether the input query terms entered are correctly added in the respective parameter arguments.
      error = function(e) {
        stop(conditionMessage(e), call. = FALSE)
      }
    )
